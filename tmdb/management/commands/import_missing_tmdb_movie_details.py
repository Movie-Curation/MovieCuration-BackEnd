...
class Command(BaseCommand):
    help = 'tmdb_movie 필드가 비어있는 KOBIS 영화의 영어 제목을 기반으로 TMDB API에서 상세 정보를 가져와 TMDB 모델에 저장합니다.'

    def handle(self, *args, **options):
        try:
            # tmdb_movie 필드가 비어있는 KOBIS 영화 가져오기
            kobis_movies = self.get_movies_without_tmdb_movie()
            total_movies = kobis_movies.count()
            self.stdout.write(self.style.NOTICE(f'총 {total_movies}개의 tmdb_movie가 비어있는 KOBIS 영화 중 TMDB 정보를 가져옵니다.'))
            ...
            self.stdout.write(self.style.SUCCESS('tmdb_movie가 비어있는 모든 KOBIS 영화에 대한 TMDB 정보 수집이 완료되었습니다.'))
    ...
    def get_movies_without_tmdb_movie(self):
        """
        tmdb_movie 필드가 비어있는(KOBIS 영화 중 TMDB 정보가 연결되지 않은) 영화들을 조회합니다.
        """
        return Movie.objects.filter(tmdb_movie__isnull=True)
